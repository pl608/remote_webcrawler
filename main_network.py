...
class Data():
    ...
    def trigger_save(self,url):
        path_ = path.join(self.dir, md5(url.encode('utf-8')).hexdigest()[:3],md5(url.encode('utf-8')).hexdigest()[:16])
        try: mkdir(path.join(self.dir, path.basename(path_)[:3]))
        except: pass
        f = open(path_, 'w')
    ...
    if m[4:] in commands:
        if m[4:] == 'connected':
            ...
            try:
                ...
                net.publish(sender+b64encode(dumps([d_[0], d_[1]]).encode('utf-8')).decode('utf-8'))
